...
/*返回的是collectionItem的数量*/
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    KSHomeModel *model = self.dataAry[section];
    return model.children.count;
}
/*返回可重用的collectionView头：用来设置产品类型*/
- (UICollectionReusableView *)collectionView:(UICollectionView *)collectionView viewForSupplementaryElementOfKind:(NSString *)kind atIndexPath:(NSIndexPath *)indexPath {
    KSHomeHeadView *headView = [collectionView dequeueReusableSupplementaryViewOfKind:kind withReuseIdentifier:Head_View forIndexPath:indexPath];//可重用的collectionView头
    KSHomeModel *model = self.dataAry[indexPath.section];
    headView.model = model;//设置collection左上角产品类型
    __weak __typeof(self) ws = self;
...
//当拖动CollectionView的时候，处理TableView,使其滚动在水平垂直。
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    lastOffSetY = scrollView.contentOffset.y;//向上滚动的偏移量
